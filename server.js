// ===============================================================================
// APEX TITAN v117.0 (OMNISCIENT TITAN OVERLORD) - ULTIMATE ENGINE
// ===============================================================================
// MERGE SYNC: v116.0 (SNIPER) + v25.1 (POOL-DEPTH SCALING) + LIVENESS SYNC
// ===============================================================================
...
// --- SAFETY: GLOBAL ERROR HANDLERS (v25.1 RESILIENCE) ---
...
// --- CONFIGURATION (v25.1 OMNISCIENT ARCHITECTURE) ---
const GLOBAL_CONFIG = {
    ...
    MARGIN_ETH: "0.015",                 // v25.1 Floor (~$50)
...
async function fetchAIAssessment(ethPrice) {
    ...
    const userQuery = `ETH: $${ethPrice}. Suggest if Titan strikes should be aggressive (BUY) or defensive (SELL).`;
    try {
...
╔════════════════════════════════════════════════════════╗
║   ⚡ APEX TITAN v117.0 | OMNISCIENT TITAN OVERLORD    ║
║   MODE: DUAL SNIPE + v25.1 POOL-DEPTH SCALING ACTIVE  ║
╚════════════════════════════════════════════════════════╝${TXT.reset}`);
...
async function initWorker(CHAIN) {
    ...
    async function safeConnect() {
        try {
            ...
            // v25.1 LIVENESS HEARTBEAT (12s Frequency)
            setInterval(async () => {
                try {
                ...
                wsProvider.on("pending", async (txHash) => {
                    ...
                    if (tx && tx.to) {
                        ...
                        if (valWei >= GLOBAL_CONFIG.WHALE_THRESHOLD) {
                             console.log(`\n${TAG} ${TXT.magenta}🚨 WHALE SPOTTED: ${formatEther(valWei)} ETH | Hash: ${txHash.substring(0, 10)}...${TXT.reset}`);
...
async function attemptOmniscientStrike(provider, wallet, iface, gasOracle, pool, ethPrice, CHAIN, mode) {
    ...
    // v25.1 DUAL SCALING: Pool-Depth (Base) OR Binary Wealth (Standard)
    if (pool && CHAIN.chainId === 8453) {
        try {
            const [res0] = await pool.getReserves();
            loanAmount = BigInt(res0) / 8n; // 12.5% Cap
        } catch (e) {
